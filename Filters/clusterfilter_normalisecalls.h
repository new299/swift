...
#ifndef SWIFT_CLUSTERFILTER_NORMALISECALLS_H
#define SWIFT_CLUSTERFILTER_NORMALISECALLS_H
...
/// Experimental, as for normalisation but only using brightest base.
template<class _prec=double>
class ClusterFilter_NormaliseCalls {
public:

  ClusterFilter_NormaliseCalls(const vector<Cluster<_prec> > &clusters,
                               _prec purity_threshold_in=0.75,
                               string source_signalid_in="CROSSTALK_CORRECTED",
                               string target_signalid_in="NORMALISE_CORRECTED",
  ...
  void initialise(const vector<Cluster<_prec> > &clusters) {
    ...
    ClusterFilter_Purity<> pf_filter(11,purity_threshold,"PHASE_CORRECTED");
    ...
    for(int base=0;base<ReadIntensity<_prec>::base_count;base++) {
      for(unsigned int cycle=0;cycle < (*(clusters.begin())).const_signal(source_signalid).size();cycle++) {   // need a better way of getting cycle count, max cycles
        ...
        for(typename vector<Cluster<_prec> >::const_iterator i = clusters.begin();i != clusters.end();i++) {
          Cluster<_prec> c = pf_filter.process(*i);
          if((c.const_signal(source_signalid)[cycle].max_base() == base) &&
              c.valid) 
          all_average_base_intensity.push_back(c.const_signal(source_signalid)[cycle].get_base(base));
          //average_base_intensity[cycle][base] += c.const_signal(source_signalid)[cycle].get_base(base);
        }
