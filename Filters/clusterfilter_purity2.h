...
#ifndef SWIFT_CLUSTERFILTER_PURITY2_H
#define SWIFT_CLUSTERFILTER_PURITY2_H

#include <string>

/// Filter based on purity, creates new clusters marked as valid if pure, invalid if not pure.
template<class _prec=double>
class ClusterFilter_Purity2 {
public:

  ClusterFilter_Purity2(int         num_bases_in       =12,    ///< Min purity with this many bases
  ...
  void process(vector<Cluster<_prec> > &clusters) {
    for(size_t n=0;n<clusters.size();n++) {
      if(clusters[n].signal(source_signal_id).size() > (num_bases+1)) {
        if(clusters[n].min_purity2(0,num_bases-1,source_signal_id) > purity_threshold) {
